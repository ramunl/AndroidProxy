...
//                serviceReceiver);
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        mFabStartProxy = activity!!.findViewById(grgr.localproxy.R.id.fab_start_proxy)
        ...
        mFabStopProxy = activity!!.findViewById(grgr.localproxy.R.id.fab_stop_proxy)
    ...
    override fun onCreateView(
        ...
        val root = inflater.inflate(grgr.localproxy.R.layout.proxy_frag, container, false)
        mUsername = root.findViewById(grgr.localproxy.R.id.euser)
        mUsername!!.setAdapter(mUserArrayAdapter)
        mPassword = root.findViewById(grgr.localproxy.R.id.epass)
        mRememberPasswordCheck = root.findViewById(grgr.localproxy.R.id.check_rem_pass)
        mLocalPortEditText = root.findViewById(grgr.localproxy.R.id.local_port)
        mGlobalProxyCheck = root.findViewById(grgr.localproxy.R.id.globCheckBox)
        if (Build.VERSION.SDK_INT > LocalProxyApplication.MAX_SDK_SUPPORTED_FOR_WIFI_CONF) {
            mGlobalProxyCheck!!.setVisibility(View.GONE)
        }
        mProfileSpinner = root.findViewById(grgr.localproxy.R.id.spinner_profiles)
        mProfileSpinner!!.setAdapter(mProfileArrayAdapter)
        mAddProfileButton = root.findViewById(grgr.localproxy.R.id.add_profile_button)
...
//        });
    ...
    override fun setUsernameEmptyError() {
        mUsername!!.error = getString(grgr.localproxy.R.string.username_empty_error)
    }

    override fun setPasswordEmptyError() {
        mPassword!!.error = getString(grgr.localproxy.R.string.password_empty_error)
    }

    override fun setProfileNoSelectedError() {
        Snackbar.make(
            mUsername!!,
            grgr.localproxy.R.string.no_profile_selected_error,
            Snackbar.LENGTH_LONG
        ).show()
    }

    override fun setLocalPortEmptyError() {
        mLocalPortEditText!!.error = getString(grgr.localproxy.R.string.proxy_port_empty_error)
    }

    override fun setLocalPortOutOfRangeError() {
        mLocalPortEditText!!.error = String.format(
            Locale.ENGLISH, getString(grgr.localproxy.R.string.profile_port_outofrange_error),
    ...
    override fun showProgressDialog(show: Boolean) {
        if (show) {
            mProgressDialog!!.setMessage(getString(grgr.localproxy.R.string.proxy_credentials_check_message))
    ...
    override fun showWrongCredentialsDialog() {
        createAlertDialog(
            getString(grgr.localproxy.R.string.proxy_credentials_error),
            getString(grgr.localproxy.R.string.proxy_credentials_error_message)
        ).show()
    }

    override fun showNetworkError() {
        createAlertDialog(
            getString(grgr.localproxy.R.string.proxy_network_error),
            getString(grgr.localproxy.R.string.proxy_network_error_message)
        ).show()
    }

    override fun showConnectionTimeOutError() {
        createAlertDialog(
            getString(grgr.localproxy.R.string.proxy_connectiontimeout_error),
            getString(grgr.localproxy.R.string.proxy_connectiontimeout_error_message)
        ).show()
    }

    override fun showUnknownHostError() {
        createAlertDialog(
            getString(grgr.localproxy.R.string.proxy_unknownhost_error),
            getString(grgr.localproxy.R.string.proxy_unknownhost_error_message)
        ).show()
    }

    override fun showConnectionError() {
        createAlertDialog(
            getString(grgr.localproxy.R.string.proxy_connection_error),
            getString(grgr.localproxy.R.string.proxy_connection_error_message)
        ).show()
    }

    override fun showErrorStartingService() {
        createAlertDialog(
            getString(grgr.localproxy.R.string.error_starting_proxy_service_title),
            getString(grgr.localproxy.R.string.local_port_not_available)
        ).show()
    }
    ...
    override fun setLocalPortNotAvailable() {
        createAlertDialog(
            getString(grgr.localproxy.R.string.port_not_available_title),
            getString(grgr.localproxy.R.string.local_port_not_available)
        ).show()
    ...
    private fun createAlertDialog(title: String, message: String): AlertDialog {
        ...
        builder.setPositiveButton(getString(grgr.localproxy.R.string.accept_text)) { dialog, which -> dialog.cancel() }
    ...
    private fun createWifiAlertDialog(): AlertDialog {
        ...
        val view = inflater.inflate(grgr.localproxy.R.layout.wifi_alert_dialog, null)
        val wifiConfigImage = view.findViewById<ImageView>(grgr.localproxy.R.id.wifiConfigImageView)
        val mAtacher = PhotoViewAttacher(wifiConfigImage)
        val dontShowCheckBox = view.findViewById<CheckBox>(grgr.localproxy.R.id.dontShowCheckBox)
        builder.setTitle(requireContext().resources.getString(grgr.localproxy.R.string.wifiSettings))
        builder.setView(view)
        builder.setPositiveButton(grgr.localproxy.R.string.wifiSettingsGoToWifi) { dialog, which ->
            mPresenter!!.goToWifiSettings(
                dontShowCheckBox.isChecked
            )
        }
        builder.setNegativeButton(grgr.localproxy.R.string.wifiSettingsContinue) { dialog, which ->
            val profileId =
                if (mProfileSpinner!!.selectedItem == null) "" else (mProfileSpinner!!.selectedItem as Profile).id
    ...
    private inner class UsersArrayAdapter(context: Context, users: ArrayList<User?>) :
        ArrayAdapter<User?>(context, grgr.localproxy.R.layout.user_list_item, users) {
        private val viewResourceId = grgr.localproxy.R.layout.user_list_item
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            val username = rowView!!.findViewById<TextView>(grgr.localproxy.R.id.user_username)
            if (user != null) {
